...
echo "-----------------------------------------------------------------------------------------------------------<br>";
echo "Enviando informaci�n del tercero: " . $_SESSION['VEC_UN_TERCERO_PER']['NUMIDTERCERO'] . " ( " . $_SESSION['VEC_UN_TERCERO_PER']['NOMIDTERCERO'] . " " . $_SESSION['VEC_UN_TERCERO_PER']['PRIMERAPELLIDOIDTERCERO'] . " )<br>";
echo "-----------------------------------------------------------------------------------------------------------<br><br>";
...
echo "<br><br>Intento n�mero: " . $_SESSION['CUENTA_INTENTOS_10'];
...
if ($error_new_soap == "no error al crear soap") {
    ...
    $conta_nombres = 0;
    foreach ($_SESSION['VEC_UN_TERCERO_PER'] as $campo) {
        if (strlen(trim($campo)) == 0) {
            // si est� en blanco, no debe enviar:
            $conta_nombres++;
        } else {
            $aux_vbles = $aux_vbles
                    . "<" . $_SESSION['NOMBRES_CAMPOS_PER'][$conta_nombres]
                    . ">"
                    . utf8_decode($campo)
                    . "</"
                    . $_SESSION['NOMBRES_CAMPOS_PER'][$conta_nombres]
    ...
    if ($_SESSION['DURACION_ESPERA'] > $_SESSION['TIEMPO_LIMITE_ESPERA_SEGUNDOS']) {
        ...
        $aux_error = "El Ministerio ha tardado m�s de " . $minutos_mensaje . " minuto(s) en responder. <br><br>Por favor intente m�s tarde o presione <a href='10b_ini_contadores.php'>aqui</a> para intentarlo ya. <br><br>El programa ha suspendido su ejecuci�n.";
        echo $aux_error;
        exit;
    } else {
        $_SESSION['ERROR_WEBSERVICES'] = "Intentando conexi�n con el Servidor del Ministerio.";
        $_SESSION['CUENTA_INTENTOS_10']++;
...
if (substr(trim($cadena_xml->ErrorMSG), 0, 18) == "Error al solicitar" or substr(trim($cadena_xml->ErrorMSG), 0, 25) == "Invalid pointer operation" or substr(trim($cadena_xml->ErrorMSG), 0, 24) == "Access violation at addr" or $error_envio_param == 'SI error al enviar parametros' or $error_new_soap == "SI error al crear soap" or substr(trim($cadena_xml->ErrorMSG), 0, 16) == "Access violation"
) {
...
// sigue el proceso de la respuesta que di� el webservice
if (strlen(trim($cadena_xml->ingresoid)) == 0 and $error_conexion == "si_existe") {
...
} elseif (strlen(trim($cadena_xml->ingresoid)) == 0 and $error_conexion == "no_existe") {
    echo utf8_decode($respuesta);
    echo "<br><br>Por favor proceda a corregir la inconsistencia en la informaci�n.";
    $aux_error = "<br><br>Una vez hecha la correcci�n, puede presionar <a href='10b_ini_contadores.php'>aqui</a> para re-enviar.";
    echo $aux_error;
    exit;
}  // fin del if c�digo nombre de error delweb service
else {
    // el envio fu� exitoso:
    ...
    include("abrebd.php");
    $aux_tipo = "ter_per";
    $aux_numero = $_SESSION['VEC_UN_TERCERO_PER']['NUMIDTERCERO'];
    ...
    $consultaagregar = mysql_query($cadenaparaagregar, $conexion) or die("Error: No se pudo agregar el n�mero de autoriz.");

    // 2)
    echo "Se envi� correctamente el tercero al Ministerio. N�mero de Autorizaci�n: " . $cadena_xml->ingresoid;
    $_SESSION['CONTADOR_REGISTROS_PER']++;
