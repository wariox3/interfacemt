<?
   ...
   echo "<br><br>Intento número: ".$_SESSION['CUENTA_INTENTOS_11'];
   ...
   echo "<br><br>";
	 ...
	 if($error_new_soap=="no error al crear soap")
	 {
       ...
       $conta_nombres=0;
			 foreach($_SESSION['VEC_UN_TERCERO_EMP'] as $campo)
			 {
             ...
             $conta_nombres++;			       
			    ...
			    {
						  $aux_vbles=$aux_vbles
							   ."<".$_SESSION['NOMBRES_CAMPOS_EMP'][$conta_nombres]
								 .">"
								 .utf8_decode($campo)
								 ."</"
								 .$_SESSION['NOMBRES_CAMPOS_EMP'][$conta_nombres]
	  ...
	  else
	  {
	     // SI hubo un error al crear new_soap:
           ...
           $aux_error="El Ministerio ha tardado más de ".$minutos_mensaje
					    ." minuto(s) en responder. <br><br>Por favor intente más 
							tarde o presione <a href='11b_ini_contadores.php'>aqui</a> 
							para intentarlo ya. <br><br>El programa ha suspendido su 
							ejecución.";   
					 echo $aux_error;
					 exit;
			 }
    ...
    $_SESSION['ERROR_WEBSERVICES']="Desde WS: ".utf8_decode(utf8_decode($cadena_xml->ErrorMSG));
    ...
    if(strlen(trim($cadena_xml->ingresoid))==0
       ...
       echo utf8_decode($respuesta);		
			 ...
			 if($_SESSION['DURACION_ESPERA'] > $_SESSION['TIEMPO_LIMITE_ESPERA_SEGUNDOS'])
			 {
			 		 $minutos_mensaje=number_format($_SESSION['TIEMPO_LIMITE_ESPERA_SEGUNDOS']/60,2,'.',',');
           $aux_error="El Ministerio ha tardado más de ".$minutos_mensaje
					    ." minuto(s) en responder. <br><br>Por favor intente 
							más tarde o presione <a href='11b_ini_contadores.php'>aqui</a> 
							para intentarlo ya. <br><br>El programa ha suspendido 
							su ejecución.";   
					 echo $aux_error;
					 exit;
			 }
       ...
       $aux_error="<br><br>Una vez hecha la corrección, puede presionar <a href='11b_ini_contadores.php'>aqui</a> para re-enviar.";   
		   echo $aux_error;
       exit;
       ...
       include("abrebd.php");  
       $aux_tipo="ter_emp";
       $aux_numero=$_SESSION['VEC_UN_TERCERO_EMP']['NUMIDTERCERO'];
       ...
       // 2)
       echo "Se envió correctamente el tercero al Ministerio. Número de Autorización: ".$cadena_xml->ingresoid;
       $_SESSION['CONTADOR_REGISTROS_EMP']++;
