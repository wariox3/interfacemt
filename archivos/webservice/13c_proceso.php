<?
   ...
   echo "<br><br>Intento número: ".$_SESSION['CUENTA_INTENTOS_13'];
   if($_SESSION['DURACION_ESPERA']!=0)
   {
	 ...
	 if($error_new_soap=="no error al crear soap")
	 {
       ...
       $conta_nombres=0;
			 foreach($_SESSION['VEC_UN_VEHICULO'] as $campo)
			 {
             ...
             $conta_nombres++;			       
			    ...
			    {
						  $aux_vbles=$aux_vbles
							   ."<".$_SESSION['NOMBRES_CAMPOS_VEH'][$conta_nombres]
								 .">"
								 .utf8_decode($campo)
								 ."</"
								 .$_SESSION['NOMBRES_CAMPOS_VEH'][$conta_nombres]
       ...
       include("encab_xml_vehiculo.php");  // arma $aux_encabezado
	  ...
	  else
	  {
	     // SI hubo un error al crear new_soap:
           ...
           $aux_error="El Ministerio ha tardado más de ".$minutos_mensaje." minuto(s) en responder. <br><br>Por favor intente más tarde o presione <a href='13b_ini_contadores.php'>aqui</a> para intentarlo ya. <br><br>El programa ha suspendido su ejecución.";   
					 echo $aux_error;
					 exit;
			 }
			 else
    ...
    if(strlen(trim($cadena_xml->ingresoid))==0
       ...
       $aux_error="<br><br>Una vez hecha la corrección, puede presionar <a href='13b_ini_contadores.php'>aqui</a> para re-enviar.";   
		   echo $aux_error;
       exit;
       ...
       include("abrebd.php");  
       $aux_tipo="veh";
       $aux_numero=$_SESSION['VEC_UN_VEHICULO']['NUMPLACA'];
       $aux_autoriz=$cadena_xml->ingresoid;
			 ...
			 $vto_cambiar=$_SESSION['VEC_UN_VEHICULO']['FECHAVENCIMIENTOSOAT'];
       ...
       // 2)
       echo "Se envió correctamente el vehículo al Ministerio. Número de Autorización: ".$cadena_xml->ingresoid;
